...
NAME_MAPPINGS = {
    'CHP Alarm Status': 'WKK_VoorAlarm',
    'CHP Analysis': 'Analysis',
    'CHP Boiler ON/OFF (BMS) Status': 'Ketel_Aan',
    'CHP Elec Auxiliairies': 'Elektr_Netto_Verbruik',
    'CHP Elec Gross Power': 'Actueel_WKK_Vermogen',
    'CHP Elec Gross Production': 'Elektr_Gen_Productie',
    'CHP Elec Net Active Power': 'Actueel_Elektr_Vermogen',
    'CHP Elec Net Production': 'Elektr_Netto_Productie',
    'CHP Gas Cons': 'Aardgas_ConV_Volume',
    'CHP Heat Demand Status': 'Warmtevraag_actief',
    'CHP Heat Production (HT)': 'HT_Warmte_Productie',
    'CHP Heat Production (LT)': 'LT_Warmte_Productie',
    'CHP Heat Production (Steam)': 'Stoom_Productie',
    'CHP Machine Room Temp': 'Cabine_Temp',
    'CHP Maintenance Status': 'Servicekey_ON',
    'CHP Manual/Auto Status': 'WKK_Automatisch',
    'CHP Motor Actif Status': 'Motor_Actief',
    'CHP Motor Starts Number': 'Aantal_Starts',
    'CHP Motor Stop Alarm Status': 'Motor_StopAlarm_origineel',
    'CHP Operating Hours': 'Draaiuren',
    'CHP Pre Alarm Status': 'WKK_VoorAlarm_origineel',
    'CHP Release Status (Customer)': 'Vrijgave_Klant_WKK',
    'CHP Total Heat Production': 'Total_Heat_Production',
    'CHP Water Buffer In Temp': 'Temp_Buffer_In',
    'CHP Water Buffer Out Temp': 'Temp_Buffer_Uit',
    'CHP Water Return Temp (Customer)': 'TempCV_Retour',
    'CHP Water Setpunt Temp (Customer)': 'Temp_Setpunt_Klant',
    'CHP Water Supply Temp (Customer)': 'TempCV_Vertrek',
    'CHP WKK Stop Alarm Status': 'WKK_StopAlarm'
}


def source_callback(source, site):
    source_key = NAME_MAPPINGS.get(source['name'], source['name'])
    source['meterNumber'] = f"{site['clientData']['Project informations']['General']['Name ref']}_{source_key}"
    return source


def get_excluded_sources(site, site_content_duplicator):
    chp_form = site.get('clientData', dict()).get('CHP', dict()).get('CHP 1 - Characteristics', dict())
    excluded = list()
    for heat_type in ('Steam', 'LT', 'HT'):
        if not chp_form.get(f"{heat_type} Production", False):
            excluded.append(f"CHP Heat Production ({heat_type})")
    return excluded if len(excluded) > 0 else None


if __name__ == '__main__':
    SiteContentDuplicator([260588], 281047,
                          source_key_field='name',
                          account_id=347,
                          source_callback=source_callback,
                          get_excluded_sources=get_excluded_sources,
                          update_variables=True).run()
